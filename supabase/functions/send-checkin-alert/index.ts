// Supabase Edge Function — send check-in alert SMS + email
//
// Sends both a Twilio SMS and a Resend email to the emergency contact
// when a rider misses a check-in.
//
...
function checkinMissedHtml(riderName: string, mapsUrl: string, checkInTime: string, timestamp: string): string {
  return `<!DOCTYPE html>
<html>
    ...
    <tr><td align="center" style="padding:24px 0;">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td style="background-color:#FFFFFF;padding:40px 32px;">
          <h1 style="color:#C62828;font-size:24px;font-weight:700;margin:0 0 16px 0;">Missed Check-In</h1>
          <div style="color:#222222;font-size:15px;line-height:24px;">
            <p style="margin:0 0 16px 0;"><strong style="color:#111111;">${riderName}</strong> has not checked in on Time to Moto.</p>
            <table cellpadding="8" style="border-collapse:collapse;width:100%;margin:0 0 16px 0;">
              <tr>
                <td style="color:#777777;padding:6px 12px 6px 0;vertical-align:top;font-size:14px;">Check-in was due</td>
                <td style="color:#222222;padding:6px 0;font-size:14px;">${checkInTime}</td>
              </tr>
              <tr>
                <td style="color:#777777;padding:6px 12px 6px 0;vertical-align:top;font-size:14px;">Alert sent</td>
    ...
    try {
      const emailRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: [contactEmail],
          subject: `SAFETY ALERT — ${riderName} missed a check-in`,
          html: checkinMissedHtml(riderName, mapsUrl, checkInTime || 'Not specified', timestamp),
          text: `TIME TO MOTO SAFETY ALERT: ${riderName} has not checked in.\n\n${checkInTime ? `Check-in was due: ${checkInTime}\n` : ''}Alert sent: ${timestamp}\nLast known location: ${mapsUrl}\n\nPlease try to reach them.`,
        }),
      });
      const emailData = await emailRes.json();
      emailId = emailData.id;
    } catch (emailErr) {
      console.error('checkin alert email error:', emailErr);
    }
  }
