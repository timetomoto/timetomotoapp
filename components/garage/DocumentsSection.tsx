...
function sortRecords(records: GarageDocument[], key: SortKey): GarageDocument[] {
  const copy = [...records];
  switch (key) {
    case 'date_desc': return copy.sort((a, b) => b.dateAdded.localeCompare(a.dateAdded));
    case 'date_asc':  return copy.sort((a, b) => a.dateAdded.localeCompare(b.dateAdded));
...
interface FormModalProps {
  visible: boolean;
  bikeId: string;
  editing: GarageDocument | null;
  onSave: (r: GarageDocument) => void;
  onClose: () => void;
}
...
function FormModal({ visible, bikeId, editing, onSave, onClose }: FormModalProps) {
  ...
  const [dateAdded, setDate]    = useState(today);
  ...
  useEffect(() => {
    if (editing) {
      ...
      setDocType(editing.documentType);
      setDate(editing.dateAdded);
      setNotes(editing.notes ?? '');
    } else {
      setTitle(''); setDocType(DOCUMENT_TYPES[0]); setDate(today); setNotes('');
    }
  }, [editing, visible]);
        ...
        <View style={[fm.sheet, { backgroundColor: theme.bgPanel, borderColor: theme.border }]}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <TextInput
              ...
              placeholder="e.g. Insurance card 2026"
              placeholderTextColor={theme.textMuted}
            />

            <Text style={[fm.label, { color: theme.textSecondary }]}>DOCUMENT TYPE</Text>
            <Pressable
              style={[fm.input, fm.pickerBtn, { backgroundColor: theme.bgCard, borderColor: theme.border }]}
              onPress={() => setShowTypePicker(true)}
            >
              <Text style={{ color: theme.textPrimary }}>{docType}</Text>
              ...
              <View style={[fm.pickerList, { backgroundColor: theme.bgCard, borderColor: theme.border }]}>
                <ScrollView nestedScrollEnabled showsVerticalScrollIndicator={false} style={{ maxHeight: 200 }}>
                  {DOCUMENT_TYPES.map((t) => (
                    <Pressable
                      key={t}
                      style={[fm.pickerItem, { borderBottomColor: theme.border }, t === docType && { backgroundColor: theme.red + '22' }]}
                      onPress={() => { setDocType(t); setShowTypePicker(false); }}
                    >
                      <Text style={[fm.pickerItemText, { color: t === docType ? theme.red : theme.textPrimary }]}>{t}</Text>
                    </Pressable>
                  ))}
                </ScrollView>
              </View>
...
export default function DocumentsSection({ bikeId, userId }: { bikeId: string; userId?: string }) {
  ...
  function handleEdit(r: GarageDocument) { setEditing(r); setShowForm(true); }

  function handleDelete(r: GarageDocument) {
  ...
  function handleBulkDelete() {
    ...
    Alert.alert(`Delete ${count} document${count > 1 ? 's' : ''}?`, 'This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
  ...
  return (
    <View style={st.root}>
      <View style={st.sectionHeader}>
        <Text style={[st.sectionTitle, { color: theme.textSecondary }]}>DOCUMENTS</Text>
